...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: recentCalls }, { data: upcomingMeetings }] =
    await Promise.all([
      supabase
        .from("profiles")
        .select("full_name")
      ...
      supabase
        .from("calls")
        .select(
          "id, about, call_status, priority, due_date, contact:people!contact_id(full_name), client:clients!client_id(full_name)"
        )
